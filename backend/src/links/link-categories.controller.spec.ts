...
describe('LinkCategoriesController', () => {
  ...
  describe('PATCH /link-categories/reorder', () => {
    it('204 — réordonne les catégories', async () => {
      ...
      expect(service.reorder).toHaveBeenCalledWith(42, [
        { id: 1, position: 0 },
      ]);
    });
  });
  ...
  describe('PATCH /link-categories/:id', () => {
    it('200 — met à jour une catégorie', async () => {
      ...
      const result = await controller.update(mockUser, 1, {
        name: 'Updated',
      });
    ...
    it('404 — lève NotFoundException si catégorie introuvable', async () => {
      service.update.mockRejectedValue(
        new NotFoundException('Catégorie 99 introuvable'),
      );
      await expect(
        controller.update(mockUser as any, 99, { name: 'X' }),
  ...
  describe('DELETE /link-categories/:id', () => {
    ...
    it('404 — lève NotFoundException si catégorie introuvable', async () => {
      ...
      await expect(controller.remove(mockUser as any, 99)).rejects.toThrow(
        NotFoundException,
      );
    });
  });
