...
describe('LinksController', () => {
  ...
  beforeEach(async () => {
    ...
    ogFetchService = {
      fetchOgData: jest.fn().mockResolvedValue({ title: 'GitHub', faviconUrl: 'https://github.com/favicon.ico' }),
    };
  ...
  it('fetchOgPreview() délègue à OgFetchService', async () => {
    ...
    expect(ogFetchService.fetchOgData).toHaveBeenCalledWith('https://github.com');
    expect(result).toEqual({ title: 'GitHub', faviconUrl: 'https://github.com/favicon.ico' });
  });
  ...
  it('create() crée un lien', async () => {
    const dto = { url: 'https://example.com', title: 'Example', categoryId: 10 };
  ...
  it('reorder() appelle LinksService.reorder', async () => {
    ...
    await controller.reorder(mockUser, dto as any);
